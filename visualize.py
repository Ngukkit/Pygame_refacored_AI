...
def visualize_argmax_actions(model, input_dim):
    ...
    cmap = plt.colormaps['tab10']  # 수정: get_cmap 대신 colormaps 직접 접근
    ...
    for action_id in unique_actions:
        ...
        plt.scatter(
            ...
            c=np.full(np.sum(idx), action_id),
            cmap=cmap,
...
def visualize_q_surface_1dim(model, input_dim, ax, action_index=1, resolution=50, slices=None):
    if input_dim < 3:
        print("⚠️ Q surface는 최소 3차원 입력 상태가 필요합니다.")
        return

    if slices is None:
        slices = [0.0, 0.5, 1.0]  # state[2] 고정값 리스트
    ...
    for idx, slice_val in enumerate(slices):
        ...
        for i in range(resolution):
            for j in range(resolution):
                ...
                state[0, 0] = X[i, j]
                state[0, 1] = Y[i, j]
                state[0, 2] = slice_val  # 슬라이스 축 고정값
                with torch.no_grad():
                    q_values = model(torch.tensor(state))
                    Z[i, j] = q_values[0, action_index].item()
        
        ax.plot_surface(X, Y, Z, alpha=0.5/(idx+1), cmap='viridis', label=f'state[2]={slice_val:.2f}')
...
def visualize_q_surface_3dim(model, input_dim, ax, action_index=3, resolution=50, slices=None):
    if input_dim < 3:
        print("⚠️ Q surface는 최소 3차원 입력 상태가 필요합니다.")
        return
...
def visualize_q_values(model_path, input_dim, output_dim):
    ...
    visualize_loss_surface(model, input_dim, output_dim,ax = dx)   
